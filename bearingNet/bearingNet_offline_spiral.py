...
if activity == "train":
        ...
        for y in range(gridSize):
            for x in range(gridSize):
                if x != nest_loc[0] or y != nest_loc[1]:
                    dist = np.sqrt((x-nest_loc[0])**2+(y-nest_loc[1])**2)
    ...
    print("Saving spiral vector, confidence, and error maps")

    # settings
    R = 0.06    # radius of the vectors
    x_interv = [538.-7.,538.+7]
    y_interv = [573.-7,573.+7]
    ...
    for i in range(amount_of_gaze_angles):
        ...
        x_coords_gaze, y_coords_gaze = Calc_coords_wrt_north(gaze_angle_lst[i*99:(i+1)*99])
        ...
        MSE_matrix = MSE.reshape(gridSize,gridSize)
        ...
        numRotations = 4
        theta_lst = np.linspace(0,numRotations*2*np.pi,num=numRotations*pointsPerRotation,endpoint=False)

        spiral_radii = spiralParam1 + spiralParam2*theta_lst
        xpos_lst = spiral_radii*np.cos(theta_lst)
        ypos_lst = spiral_radii*np.sin(theta_lst)
        origins = [[],[]]   # constructing list to hold different values for spiral coords
        
        for j in range(len(xpos_lst)):
            circle = plt.Circle((nest_loc[0],nest_loc[1]),0.1,fill=False)
            ax.add_artist(circle)
            if -ypos_lst[j] != nest_loc[0] or xpos_lst[j] != nest_loc[1]:
                origins[0].append(-ypos_lst[j])
                origins[1].append(xpos_lst[j])

        # Vector plot
        plt.scatter(TreePos[:5,0]-538.,TreePos[:5,1]-573.,s=np.array(TreeFoliageRadius)*72/0.0254/4,color='green',alpha=0.2)        # conversion from m to points
        plt.scatter(TreePos[:5,0]-538.,TreePos[:5,1]-573.,color='green',marker='2')     # only the first tree positions correspond with actual trees, x and z in scene are y and x in flightmare
        plt.quiver(*origins,x_coords,y_coords,scale=1,color='navy')
        plt.xlim([-7,7])
        plt.ylim([-7,7])
        plt.plot(nest_loc[0],nest_loc[1],'o',color='red')

        
        # we need to make a grid dependent on the smallest distance in the spiral
        # https://stackoverflow.com/questions/31667070/max-distance-between-2-points-in-a-data-set-and-identifying-the-points
        grid_points = []
        for j in range(len(origins[0])):
            grid_points.append([origins[0][j],origins[1][j]])
        grid_points = np.array(grid_points)

        hull = ConvexHull(grid_points)
        hullpoints = grid_points[hull.vertices,:]
        hdist = cdist(hullpoints, hullpoints, metric='euclidean')
        bestpair = np.unravel_index(hdist.argmax(), hdist.shape)
        
        max_grid_dist = np.sqrt((hullpoints[bestpair[1]][0]-hullpoints[bestpair[0]][0])**2+(hullpoints[bestpair[1]][1]-hullpoints[bestpair[0]][1])**2)
        min_grid_dist = np.sqrt(grid_points[0][0]**2+grid_points[0][1]**2)
        # get the dimensions that is necessary for the grid
        grid_dim_extra = 50
        grid_dim = int(np.ceil(max_grid_dist/min_grid_dist))+grid_dim_extra
        # scale the spiral points according to grid dims
        x_mesh_spiral = (np.round(np.insert(origins[0],0,0.)/min_grid_dist)).astype(int)
        y_mesh_spiral = (np.round(np.insert(origins[1],0,0.)/min_grid_dist)).astype(int)
        # define offsets for spacing
        MSE_x_offset = int(89 + grid_dim_extra/2)
        MSE_y_offset = int(91 + grid_dim_extra/2)
        ...
        confidence = np.insert(confidence,0,0.)     # confidence indication as unit deviation
        # use the scaled coords as indices to insert into conf values
        conf_matrix = np.zeros((grid_dim,grid_dim))

        for j in range(len(x_mesh_spiral)):
            # real position with some offset to be in the middle of the grid
            conf_matrix[y_mesh_spiral[j]+MSE_x_offset,x_mesh_spiral[j]+MSE_y_offset] = confidence[j]      # row nr = y coord and same same for x coords
            # extra cells to improve visibility
            for overflow in range(1,3):
                conf_matrix[y_mesh_spiral[j]+MSE_x_offset+overflow,x_mesh_spiral[j]+MSE_y_offset] = confidence[j]
                conf_matrix[y_mesh_spiral[j]+MSE_x_offset,x_mesh_spiral[j]+MSE_y_offset+overflow] = confidence[j]
                conf_matrix[y_mesh_spiral[j]+MSE_x_offset-overflow,x_mesh_spiral[j]+MSE_y_offset] = confidence[j]
                conf_matrix[y_mesh_spiral[j]+MSE_x_offset,x_mesh_spiral[j]+MSE_y_offset-overflow] = confidence[j]

                conf_matrix[y_mesh_spiral[j]+MSE_x_offset+overflow,x_mesh_spiral[j]+MSE_y_offset+overflow] = confidence[j]
                conf_matrix[y_mesh_spiral[j]+MSE_x_offset-overflow,x_mesh_spiral[j]+MSE_y_offset+overflow] = confidence[j]
                conf_matrix[y_mesh_spiral[j]+MSE_x_offset+overflow,x_mesh_spiral[j]+MSE_y_offset-overflow] = confidence[j]
                conf_matrix[y_mesh_spiral[j]+MSE_x_offset-overflow,x_mesh_spiral[j]+MSE_y_offset-overflow] = confidence[j]
        ...
        plt.xticks([16,50,84,118,152,186,220],np.arange(-6,8,2).astype('str'))
        plt.yticks([16,50,84,118,152,186,220],np.arange(-6,8,2).astype('str'))
        im = ax.pcolormesh(np.arange(grid_dim+1), np.arange(grid_dim+1), conf_matrix, shading='flat', vmin=conf_matrix.min(), vmax=conf_matrix.max(),cmap=cm.binary)
        ...
        ax.add_patch(Rectangle((nest_loc[0]+MSE_x_offset+2,nest_loc[1]+MSE_y_offset-2), 1, 1, fc='none', ec='red', lw=2))
        
        
        # MSE error plot settings
        MSE_matrix = np.zeros((grid_dim,grid_dim))  # use the scaled coords as indices to insert into MSE values

        for j in range(len(x_mesh_spiral)):
            # real position with some offset to be in the middle of the grid
            MSE_matrix[y_mesh_spiral[j]+MSE_x_offset,x_mesh_spiral[j]+MSE_y_offset] = MSE[j]      # row nr = y coord and same same for x coords
            # extra cells to improve visibility
            for overflow in range(1,3):
                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset+overflow,x_mesh_spiral[j]+MSE_y_offset] = MSE_angle[j]
                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset,x_mesh_spiral[j]+MSE_y_offset+overflow] = MSE_angle[j]
                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset-overflow,x_mesh_spiral[j]+MSE_y_offset] = MSE_angle[j]
                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset,x_mesh_spiral[j]+MSE_y_offset-overflow] = MSE_angle[j]

                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset+overflow,x_mesh_spiral[j]+MSE_y_offset+overflow] = MSE_angle[j]
                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset-overflow,x_mesh_spiral[j]+MSE_y_offset+overflow] = MSE_angle[j]
                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset+overflow,x_mesh_spiral[j]+MSE_y_offset-overflow] = MSE_angle[j]
                MSE_matrix[y_mesh_spiral[j]+MSE_x_offset-overflow,x_mesh_spiral[j]+MSE_y_offset-overflow] = MSE_angle[j]
        ...
        plt.xticks([16,50,84,118,152,186,220],np.arange(-6,8,2).astype('str'))
        plt.yticks([16,50,84,118,152,186,220],np.arange(-6,8,2).astype('str'))
        im = ax.pcolormesh(np.arange(grid_dim+1), np.arange(grid_dim+1), MSE_matrix, shading='flat', vmin=MSE_matrix.min(), vmax=MSE_matrix.max(),cmap=cm.binary)
        axins = inset_axes(ax,width="5%",height="100%",loc='center right',borderpad=-2)
        cbar = plt.colorbar(im,cax=axins)
    ...
    print("Save positions and gaze angles in epoch")

    with open("labels_in_epoch.md","r") as f:
        labels_in_epoch_string = f.read()

    labels_in_epoch = [name for name in labels_in_epoch_string.split("'") if name[0]=="r"]

    gaze_angles_in_epoch = []
    positions_in_epoch = []
    for label in labels_in_epoch:
        image_id = label.split("-")[-1].split(".")[0]

        positions_in_epoch.append(int(image_id[-2:]))

        if len(image_id) == 2:
            gaze_angles_in_epoch.append(0)
        else:
            gaze_angles_in_epoch.append(int(image_id[:-2]))

    plt.figure()
    plt.hist(positions_in_epoch,bins=np.arange(100)-0.5)
    plt.xticks(np.linspace(0,100,21,True),rotation=90)
    ax = plt.gca()
    ax.grid(axis="x")
    plt.savefig("positions_in_epoch.png",bbox_inches='tight',pad_inches=0.2)

    plt.figure()
    plt.hist(gaze_angles_in_epoch,bins=np.arange(360)-0.5)
    plt.xticks(np.linspace(0,360,31,True),rotation=90)
    ax = plt.gca()
    ax.grid(axis="x")
    plt.savefig("gaze_angles_in_epoch.png",bbox_inches='tight',pad_inches=0.2)

    ###############################################################################################################################
    # Plot MSE error wrt label
    print("Save MSE wrt label plot")

    # make a list that has [id,label_angle,angle_MSE_error]
