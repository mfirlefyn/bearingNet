...
for dist in [1]: 	
	...
	os.chdir("./bearingNet")
	subprocess.call(["python3", "writeLabels.py", "--nest_location_x", str(x), "--nest_location_y", str(y), "--grid_distance", str(dist), "--training_trajectory", "rectGrid"])
	os.chdir("./..")
	...
	os.chdir("./bearingNet")
	subprocess.call(["python3", "bearingNet_offline_rectGrid.py", "--activity", "train", "--nest_location_x", str(x), "--nest_location_y", str(y)])
	os.chdir("./..")

	# run the logging script to log the train results (run and weight)
	print(f"({x},{y},{dist})+++++LOG THE NET WEIGHTS AND TENSORBOARD RUN+++++")
	os.chdir("./logging")
	subprocess.call(["python3", "logRun_offline_rectGrid.py", "--nest_location_x", str(x), "--nest_location_y", str(y), "--grid_distance", str(dist)])
	os.chdir("./..")

	# run the bearingNet.py script in order to evaluate the network
	print(f"({x},{y},{dist})+++++EVALUATE THE NETWORK+++++")
	os.chdir("./bearingNet")
	subprocess.call(["python3", "bearingNet_offline_rectGrid.py", "--activity", "evaluate", "--nest_location_x", str(x), "--nest_location_y", str(y)])
	os.chdir("./..")
